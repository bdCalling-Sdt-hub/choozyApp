...
const StatusScreen = ({navigation} : NavigProps<null>) => {
  return (
    <View style={tw`flex-1 bg-base`}>
     ...
     <FlatList showsVerticalScrollIndicator={false} contentContainerStyle={tw`gap-2 mt-3 pb-6`} data={postData.posts} renderItem={({item})=>{
      return(
        <>
          {/*================= Post Card ============ */}
      <View style={tw` p-4 bg-white`}>
        <TouchableOpacity activeOpacity={0.5} style={tw`flex-row gap-2 items-center self-start`}>
          <FastImage
        ...
        {
          item.content.text &&  <View style={tw`py-3`}>
      ...
      <View style={tw`flex-row items-center gap-3`}>
        <Text style={tw`text-color-Black1000  font-NunitoSansRegular text-xs`}>{item.content.views} views</Text>
      
        <Text numberOfLines={1} style={tw`text-gray-400 font-NunitoSansRegular w-[70%] text-xs`}>
        • Liked by <Text style={tw`text-color-Black1000 font-NunitoSansBold`}>{item?.content?.liked_by[0].name} and {item.content.likes} others</Text> 
        </Text>
      </View>
